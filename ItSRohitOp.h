...
// Created By @ItSRohitOp

static std::string credit = "Credit Showing After Login Key";
static std::string mod_status = "Server Disconnected";
static std::string modname = "Welcome bro, Please Login";
static std::string SLOT = "Its Showing After Login Your Key";
static std::string RohitEXP = "Welcome bro, Please Login";
...
if (curl) {
std::string api_key = OBFUSCATE("https://powercheat.hertzai.in/connect");
if (api_key != api_key) {
...
char data[4096];
sprintf(data, OBFUSCATE("game=FREEFIRE&user_key=%s&serial=%s"), user_key, UUID.c_str());
...
if (rng + 30 > time(0)) {
std::string auth = OBFUSCATE("FREEFIRE");
